...
class Queue : public Queue_HAL<T> {
   public:
    Queue(uint32_t queueLength) {
        this->handle = xQueueCreate(queueLength, sizeof(T));
    }
    ~Queue() { vQueueDelete(this->handle); }
    unsigned waiting() const override {
        return uxQueueMessagesWaiting(this->handle);
    }
    unsigned available() const override {
    ...
    bool empty_ISR() override {
        return xQueueIsQueueEmptyFromISR(this->handle);
    }
    unsigned waiting_ISR() override {
        return uxQueueMessagesWaitingFromISR(this->handle);
    }
